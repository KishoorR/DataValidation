...
def validate_csv_against_modules(csv_in: Path, module_entries):
    ...
    for row in rows:
        ...
        if not matching_entries:
            ...
            result = 'Component Not Found in Text File'
            row['Result'] = result
            # Track missing component for popup summary
            if comp_name.strip():
                missing_components.add(comp_name.strip())
            continue
    ...
    return rows, fieldnames,missing_components
...
def validate_ui(text_file, csv_file):
    ...
    # Validate CSV
    rows, fieldnames, missing_components = validate_csv_against_modules(csv_path, module_entries)
